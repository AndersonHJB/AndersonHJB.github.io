...
2. **合并多个文件**：你可以在 `cat` 命令后指定多个文件名，以便将这些文件的内容合并并显示在终端中。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/8e/8e4c97a6c0c4d516056331e715df0e3fb2bb1883a1431963c1b4dbc76518f82c.png)
...
3. **创建新文件**：如果你想将多个文件的内容合并并保存到一个新文件中，你可以使用重定向操作符 `>`。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/a9/a95d02b925f35cd839165ca6f6d94303bce2e870455ffd2692ea04d4253686da.png)
...
5. **显示行号**：如果你想在显示文件内容时显示行号，你可以使用 `-n` 或 `--number` 参数。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/fb/fba8b87052174d128878109e240071d38958d131ef9d25240c9760850dce969d.png)
...
3. **使用转义字符**：`echo` 命令支持一些转义字符，比如 `\n`（新行），`\t`（制表符）等。要启用这些转义字符，你需要使用 `-e` 参数。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/dd/dd8c5bb62306c065c4f33e97b2f6864fda9d61aa79b2f229da541b8c627dc067.png)
...
4. **不输出尾部的新行**：`echo` 命令默认会在输出的末尾添加一个新行。如果你不想输出这个新行，可以使用 `-n` 参数。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/10/1002c9e2da20c6500a0c7b0ba0cf4aceeae2ca4a46d7d0a1a8c36d21d2355b01.png)
...
1. **基本的排序**：最简单的 `sort` 命令用法就是对文件中的所有行进行排序。例如，假设我们有一个文件 `file.txt`，其内容如下：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/6e/6e05c16e21d4f3f728eeb275f373b4e63e799c36fb87d1b32072f48a3ffbcab8.png)
...
2. **数值排序**：默认情况下，`sort` 命令会按照字母顺序进行排序。如果你想按照数值进行排序，可以使用 `-n` 选项。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/f3/f35d070400bcde910f6241c1e079e0d305fece349120606b9029abbd7015afe1.png)
...
1. **链接多个命令**：你可以使用管道链接多个命令。例如，你可以使用 `ls` 命令列出目录的内容，然后使用 `grep` 命令搜索特定的文件。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/71/712133d62150f22a79e8dbd85e1a79e10dd03afe4a1f57d68d41cb3fa8a14fa5.png)
...
2. **使用 `sort` 和 `uniq` 命令删除重复的行**：你可以使用管道链接 `sort` 和 `uniq` 命令来删除文件中的重复行。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/a6/a6a27df514fbf9b72a529151acf8c553161446aa1db6c9e7f0e8d6112f048d39.png)
...
1. **基本的搜索**：最基本的 `grep` 用法就是在一个或多个文件中搜索特定的文本。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/15/153c85d00c8cb979ddad808e59accee6d57a1d0408b730188690e7e6c10e2d5a.png)
...
1. **基本用法**：最简单的 `uniq` 用法是直接使用它来过滤掉重复的行。例如：
    ...
    ![不是说只去除重复相邻的行么？——后面讲解](https://blog.images.bornforthis.cn/docs-images/sha256/7e/7efd6ec9488f916a32a05b1b0ea0f2b733fd364ede9911c13882553e19dd73d0.png)
...
2. **显示每行出现的次数**：如果你想知道每行出现的次数，你可以使用 `-c` 选项。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/f7/f7e90b9ec13208bc5202b5e01275318d4ec40a5bbcdbcee3598692e63c05115e.png)
...
3. **只显示唯一的行**：如果你只想显示出现一次的行（即不重复的行），你可以使用 `-u` 选项。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/50/5082520761ae24149ea5e7889cd41b5a387cb69dee95cd58b2fcb90e5ab4b08f.png)
...
4. **只显示重复的行**：如果你只想显示至少出现两次的行（即重复的行），你可以使用 `-d` 选项。例如：
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/53/53ef9eef4d0f24c39047b55583590cbb0ffd70f67794692e893258f030cad9ec.png)
...
![](https://blog.images.bornforthis.cn/docs-images/sha256/83/8312448ff3f67a50efb16ff3b55dedf0f971eddec059440b559994dfab3a6fbb.png)
...
![](https://blog.images.bornforthis.cn/docs-images/sha256/77/7706633be97101ab1461c22b1c92c25b34ddef161023f34e80d555b6be5a5570.jpg)

每个文件的属性由左边第一部分的 10 个字符来确定（如下图）。

![](https://blog.images.bornforthis.cn/docs-images/sha256/e5/e5ca141d2001f1b73052ca72a71ac812cd554cf25c03a01d83824891c569e528.png)
...
1. **更改文件或目录的所有者**：你可以使用 `chown` 命令 followed by the name of the new owner and the file or directory name to change the ownership. For example:
    ...
    ![](https://blog.images.bornforthis.cn/docs-images/sha256/b9/b9394a79b038638a1f9d2e97bfe5d5779814c36d7e7b09d1844c9e0a826e4e97.png)
