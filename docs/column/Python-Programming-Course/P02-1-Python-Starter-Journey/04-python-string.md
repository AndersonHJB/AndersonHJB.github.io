...
::: tip 补充启发，夏思涵 2026 年 8 月 4 日 20:00

看懂代码逻辑很重要，虽然上面的代码实现了个位数的加减乘除。但仔细研究代码逻辑以及测试后，不难发现问题：**不支持十位数以上的数字**。

同理：如果不懂代码、逻辑，AI 就算写了代码，你也无法知道 AI 给的代码是否逻辑完整！（换句话说：没有提前了解所有情况，了解所有情况的是我们自己！）

此时，如果我们不懂代码，全权交给 AI。那就会酿成大祸！

:::

### 11.3 解答

::: tip 代码讲解补充，夏思涵 2026 年 8 月 4 日 21:19:00

```python
nums = input("输入两个整数，并以空格间隔:>>>")  # 为符合题目要求：只能使用一个 input 函数，改进输入提示
print(nums)
print(f"得到用户输入: {nums}")  # 输出观察
```

- 目标：（大目标）为了实现计算两个整数的结果，（小目标）首先需要得到两个整数。（先决条件，在完成大目标前所需要做的准备！）
- 👉接着往下思考：如何得到两个数字！
- 👉如何观察出特点：我们自己是如何看懂字符串是否拥有两个整数！——>空格（也就是：我们人是如何区分两个整数）
- 特点：两个数字由【空格】间隔！
- 👉接着思考：如何得到两个整数——>要以空格进行【分隔】——>分隔，自然而然想到：`.split()`

```python
num_list = nums.split()
# 查看分隔后的结果
print(f"分割后的结果：{num_list}")
# 从结果可知，提取列表两个元素并强制转换即可进行运算
num1 = int(num_list[0])
num2 = int(num_list[1])
result = num1 + num2
print(result)
