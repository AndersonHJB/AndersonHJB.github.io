...
t.speed(0)  # 设置最快速度
t.pensize(2)  # 设置线条宽度
t.fillcolor('red')  # 设置正六边形填充颜色为红色
t.pencolor('black')  # 设置线条颜色为黑色

# 定义正六边形的边长
side_length = 50
hex_height = math.sqrt(3) * side_length / 2
...
# 绘制等边三角形并填充蓝色
def draw_triangle():
    t.fillcolor('blue')  # 设置等边三角形填充颜色为蓝色
    t.begin_fill()
    for _ in range(3):
        t.forward(side_length)
        t.left(120)
...
def draw_pattern():
    for j in range(4):
        for k in range(4):
            draw_hexagon()  # 画出红色填充的正六边形
            t.up()
            t.forward(100)  # 移动到下一个六边形位置
            t.down()

        t.up()
        t.home()
        t.right(90)
        t.forward(86.6 * (j + 1))  # 向下移动一行
        t.down()
        t.left(90)
