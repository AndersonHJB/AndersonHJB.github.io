...
一开始是实现手动截图，并保存到指定路径下，并使用 Python 的脚本实现自动上传 GitHub，然后我这边同步 clone 下载 GitHub 仓库内容，作答后微信发送答案，这样的后台操作规避了考试监测系统。但在某一次考试中，虽然没有被判定作弊，但被教授发 Email 警告！

之前还实现了一个截图后，自动读取粘贴板 and 自动发送 Email 的程序，来辅助哥伦比亚大学学生考试。此程序，暂不发布～
